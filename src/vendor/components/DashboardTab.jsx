...
function DashboardTab() {
  const { mode, products, productToEdit, updateProduct, deleteProduct } =
    useProduct();
  const { currentUser } = useAuth();
  const productsToShow = products.filter(
    (product) => product.vendorEmail == currentUser.email
  );
  ...
  const getOrders = () => {
    try {
      const q = query(
        ...
        where("product.vendorEmail", "==", `${currentUser.email}`)
      );
                              ...
                              <div className=" flex gap-2">
                                ...
                                >
                                  ...
                                  <Link to={"/updateproduct"}>
                                    <div onClick={() => productToEdit(product)}>
                                      <svg
                                        xmlns="http://www.w3.org/2000/svg"
                                        fill="none"
                                        viewBox="0 0 24 24"
